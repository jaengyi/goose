---
title: Fetch 확장 프로그램
description: Fetch MCP 서버를 goose 확장 프로그램으로 추가하기
---
...
:::warning 알려진 제한 사항
Fetch 확장 프로그램은 JSON 스키마에서 `format: uri`를 사용하기 때문에 Google 모델(예: gemini-2.0-flash)과 [작동하지 않습니다](https://github.com/block/goose/issues/1184). Google은 이를 지원하지 않습니다.
:::

이 튜토리얼에서는 웹에서 콘텐츠를 검색하고 처리하기 위해 [Fetch MCP 서버](https://github.com/modelcontextprotocol/servers/tree/main/src/fetch)를 goose 확장 프로그램으로 추가하는 방법을 다룹니다.

:::tip 요약
<Tabs groupId="interface">
  <TabItem value="ui" label="goose Desktop" default>
  [설치 프로그램 실행](goose://extension?cmd=uvx&arg=mcp-server-fetch&id=fetch&name=Fetch&description=Web%20content%20fetching%20and%20processing%20capabilities)
  </TabItem>
...
<Tabs groupId="interface">
  ...
  <GooseDesktopInstaller
    extensionId="fetch"
    extensionName="Fetch"
    description="웹 콘텐츠 가져오기 및 처리 기능"
    command="uvx"
...
Fetch 확장 프로그램을 사용하여 [Official Joke API](https://official-joke-api.appspot.com/random_joke)에서 랜덤 프로그래밍 농담을 가져와 보겠습니다.

:::info
이 API는 API 키가 필요하지 않지만 많은 API는 필요합니다. API 키를 제공해야 하는 경우 환경 변수로 설정하고 goose에게 이름을 알려줄 수 있습니다. 예: _"weatherapi에서 뉴올리언스의 내일 날씨를 가져와. 키를 직접 사용하지 말고 환경 변수 WEATHER_API_KEY에서 값을 가져오는 스크립트를 사용해"_
:::
...
![goose가 만든 웹사이트](../assets/guides/programmer-jokes-fetch-mcp.png)
