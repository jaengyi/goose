---
title: Filesystem 확장 프로그램
description: Filesystem MCP 서버를 goose 확장 프로그램으로 추가하기
---
...
이 튜토리얼에서는 강력한 코드 분석 및 파일 관리를 활성화하기 위해 [Filesystem MCP 서버](https://github.com/modelcontextprotocol/servers/tree/HEAD/src/filesystem)를 goose 확장 프로그램으로 추가하는 방법을 다룹니다. 이 확장 프로그램을 사용하면 goose가 프로젝트 구조를 분석하고, 파일을 편집 및 정리하고, 사용하지 않는 종속성을 감지하고, 소프트웨어 유지보수성을 개선하기 위한 문서를 생성할 수 있습니다.
...
:::tip 요약
  **Command**
  ...
  ```
    ...
    허용된 디렉토리를 공백으로 구분하여 여러 개 지정할 수 있습니다.
:::
...
<Tabs groupId="interface">
  ...
  설치 후, 실제 허용된 디렉토리 경로로 확장 프로그램을 업데이트하세요:
  1. 왼쪽 상단의 <PanelLeft className="inline" size={16} /> 버튼을 클릭하여 사이드바 열기
  2. 사이드바에서 `Extensions` 클릭
  3. `Filesystem` 카드에서 <Settings className="inline" size={16} /> 버튼 클릭
  4. 명령을 편집하여 플레이스홀더 경로를 공백으로 구분된 허용된 디렉토리 경로로 교체
  5. `Save Changes` 클릭
  ...
  <TabItem value="cli" label="goose CLI">
    <CLIExtensionInstructions
      ...
      commandNote={
        <>
          <code>/path/to/allowed/directory</code>를 액세스를 허용하려는 디렉토리의 실제 경로로 교체하세요. 공백으로 구분하여 여러 디렉토리를 지정할 수 있습니다.
        </>
      }
    />
  </TabItem>
...
이 예제에서는 goose를 사용하여 결제 처리 시스템, 인증 서비스, 이커머스 대시보드를 포함한 여러 프로젝트에 걸쳐 포괄적인 보안 감사를 수행합니다. 보안 취약점을 식별하고 수정 사항을 구현하며 민감한 데이터와 작업이 적절하게 보호되도록 해야 합니다.
